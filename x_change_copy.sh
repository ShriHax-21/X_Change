...
auto_select_interface() {
    ...
    # Get all non-lo, non-virtual interfaces that have an IPv4 address
    interfaces=$(ip -4 -o addr show up | awk '!/ lo / && !/docker|vir|veth|br-/{print $2}' | sort -u)
    if [ -z "$interfaces" ]; then
        echo -e "${RED}[!] No usable network interfaces found.${NC}"
        exit 1
    fi
    echo "$interfaces"
    read -p "Enter interface to use [default: first in list]: " iface
    if [ -z "$iface" ]; then
        iface=$(echo "$interfaces" | head -n 1)
    fi
    # Double check that interface is up and has an IPv4 address
    if ! ip -4 addr show "$iface" up | grep -q "inet "; then
        echo -e "${RED}[!] Interface $iface is not active and has no IPv4 address.${NC}"
        exit 1
    fi
